export const day3 = {
  ...
  subtitle: '可选日出 + Hotel Bridge 寄存行李 + Route 7',
  theme: '日出 + 徒步',
  hotel: 'Hotel Bridge Seogwipo',
  reminder: '今天开始连续徒步日。出发前确认鞋袜、补水、碳水和能量零食；不使用行李转运，自己把行李带到 Hotel Bridge 前台寄存后再轻装走 Route 7。',
  verificationNotes: [
    '9/28 日出时间接近 06:25 的旧计划值，出发前需按当天天文时间再确认。',
    'Route 7 当前官方里程与具体起终点以出发前官方 Olle 信息复核；项目暂沿用此前讨论的约 12.9 km 版本。',
    'T-1 向 Hotel Bridge 确认入住前可寄存行李；若政策临时变化，再启用行李配送作为备选。',
  ],
  planOptions: [
    {
      id: 'sunrise',
      label: '🌅 Plan A · 看日出',
      description: '早起看城山日出峰；行李先短存 Playce 前台，早餐后取回并带去 Hotel Bridge 寄存，再开始 Route 7。',
      nodeOverrides: {},
    },
    {
      id: 'sleep',
      label: '😴 Plan B · 不看日出',
      description: '多睡一会儿，退房后直接带行李吃早餐、转场 Hotel Bridge 寄存，再走 Route 7；后续时间整体更宽松。',
      hiddenNodeIds: ['sunrise'],
      nodeOverrides: {
        checkout: { time: '07:20', title: 'Playce Camp 退房 + 带行李', description: '退房后直接带上行李，不使用配送，也不需要日出时的前台短存。' },
        breakfast: { time: '07:35' },
        transfer: { time: '08:20' },
        luggageDrop: { time: '10:05' },
  ...
  timeline: [
    {
      id: 'checkout', time: '05:20', title: 'Playce Camp 退房 + 行李短存', icon: '🧳',
      description: 'Plan A 先退房，把大件行李短时寄存在 Playce 前台，轻装去看日出；早餐后取回行李再向西归浦出发。',
    ...
    },
    {
      ...
      description: '早餐候选包含 동뜬식당；以当天营业状态为准。吃完取回 Playce 的行李。',
    },
    {
      id: 'transfer', time: '08:00', title: '带行李前往西归浦 / Hotel Bridge', icon: '🚌',
      description: '自己带行李完成长距离转场。优先公共交通；若衔接差、拖箱不方便再考虑 Taxi。',
    },
    {
      id: 'luggageDrop', time: '09:45', title: 'Hotel Bridge · 前台寄存行李', icon: '🧳',
      description: '先把行李寄存在当天入住的 Hotel Bridge，再轻装前往附近的 Route 7 起点；这样不需要额外购买行李转运。',
      transportToNext: '酒店与当前 Route 7 起点区域很近，按 Naver 当天导航步行前往。',
    },
    {
    ...
    },
    {
      ...
      description: '完成后返回 Hotel Bridge 办入住、取寄存行李，不额外加景点。',
    },
    {
      id: 'bridge', time: '16:00', title: 'Hotel Bridge Seogwipo', icon: '🏨',
      description: '入住、取寄存行李、洗澡休息。',
    },
    {
  ...
  places: [
    {
      ...
      lat: 33.449, lng: 126.9188, address: 'Dongnyuam-ro, Seongsan-eup, Seogwipo-si', note: 'Day 3 出发酒店；Plan A 日出期间大件行李先短存前台。', approximate: true,
    },
    {
    ...
    {
      id: 'bridge', name: 'Hotel Bridge Seogwipo', koreanName: '호텔브릿지 서귀포', category: 'hotel', status: 'fixed', routeOrder: 3,
      lat: 33.2455925, lng: 126.5666523, address: '436 Taepyeong-ro, Seogwipo-si, Jeju-do', note: '9/28、9/29 住宿；9/28 先寄存行李，再去 Route 7。',
    },
    {
      id: 'route7Start', name: 'Olle Route 7 起点区域', category: 'hike', status: 'fixed', routeOrder: 4,
      lat: 33.244, lng: 126.565, address: 'Seogwipo-si', note: '距离 Hotel Bridge 很近；当前为路线区域约略点，出发前按官方最新路线复核。', approximate: true,
    },
    {
      id: 'beophwan', name: '法还浦', koreanName: '법환포', category: 'hike', status: 'fixed', routeOrder: 5,
      lat: 33.234, lng: 126.515, address: 'Beophwan-dong, Seogwipo-si', note: 'Route 7 中段午饭 / 补给点。', approximate: true,
    },
    {
      id: 'route7Finish', name: 'Route 7 终点区域', category: 'hike', status: 'fixed', routeOrder: 6,
